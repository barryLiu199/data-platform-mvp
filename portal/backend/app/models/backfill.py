...
class BackfillTask(Base):
    ...
    # pending / running / succeeded / failed / stopping / stopped / timeout
    ...
    ds_command_type = Column(String(32), comment="DS 执行类型，如 COMPLEMENT_DATA")
    ds_command_id = Column(BigInteger, comment="DS complement 命令 ID")
...
class BackfillInstance(Base):
    ...
    ds_instance_id = Column(BigInteger, comment="DS process instance id")
